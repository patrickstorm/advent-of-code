...
const setActionableSteps = (input) => {
  Object.keys(input).forEach((step) => {
    if (step.done || step.beingWorkedOn) {
...
const getNextActionableStep = (input) => {
  const actionable = Object.keys(input).reduce((acc, step) => {
    if (
      input[step].actionable &&
      !input[step].done &&
      !input[step].beingWorkedOn
    )
      acc.push(step);
    return acc;
  }, []);
...
class Worker {
  ...
  startWorking(step, timeNow) {
    ...
    this.workingOn = step;
    formattedInput2[step].beingWorkedOn = true;
    // (step, this.startedWorking, this.workingTil);
  }
...
class WorkForce {
  ...
  assignWorkers(timeIndex) {
    if (
      ...
      const step = this.actionable[0];
      this.steps.splice(this.steps.indexOf(step), 1);
      const availableWorker = this.findAvailableWorker();
      if (availableWorker) {
        availableWorker.startWorking(step, timeIndex);
      }
      this.actionable.shift();
      this.assignWorkers(timeIndex);
    }
  }

  updateStillWorking() {
    if (!this.stillWorking && this.steps.length) return;
    const allWorkersDone = this.workers.every((worker) => !worker.isWorking);
    if (allWorkersDone) this.stillWorking = false;
  }

  logTick(timeIndex) {
    const currentSteps = this.workers.reduce((acc, worker) => {
      if (worker.isWorking) acc.push(worker.workingOn);
      return acc;
    }, []);
    console.log('Time:', timeIndex, ' - Active Steps:', currentSteps);
  }
  ...
  tick(timeIndex) {
    ...
    this.actionable.sort();
    this.assignWorkers(timeIndex);
    this.updateStillWorking();
    // this.logTick(timeIndex);
  }
}
...
const formattedInput2 = getFormattedInput(input);
setActionableSteps(formattedInput2);

const workForce = new WorkForce(NUM_WORKERS, formattedInput2, steps);
console.log(steps);
workForce.startWork();
const secondPassed = workForce.howLongDidItTake();
